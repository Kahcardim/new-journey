...
const paths = [
  ["CAPS AD e rede pública","Acompanhamento territorial e gratuito pelo SUS, com equipe multiprofissional.","Quero entender esta opção","caps-ad"],
  ["Atendimento ambulatorial","Consultas e terapias sem afastamento integral da rotina, quando há segurança e adesão.","Quero avaliar este caminho","ambulatorial"],
  ["Acolhimento residencial","Ambiente protegido e rotina estruturada, conforme modalidade e indicação.","Quero saber se faz sentido","acolhimento-residencial"],
  ["Internação clínica","Pode ser considerada quando cuidado intensivo é necessário, sempre com avaliação médica.","Quero orientação sobre internação","internacao-clinica"],
];
...
export default function Tratamentos(){return <SiteShell>
  ...
  <section id="caminhos" className="priority-panel" aria-labelledby="caminhos-titulo">
    <div className="priority-panel-head"><div><span className="kicker">CAMINHOS DE CUIDADO</span><h2 id="caminhos-titulo">Comece pelas possibilidades.</h2></div><p>Essas opções são o conteúdo principal desta página. Escolha a que mais se aproxima da situação atual e leve a dúvida diretamente para uma conversa.</p></div>
    <div className="priority-grid treatment-action-grid">{paths.map(([title,text,cta,type],i)=><article className="action-card" key={title}><span>0{i+1}</span><h3>{title}</h3><p>{text}</p><Link className="card-cta" href={`/agendamento?origem=tratamentos&tipo=${type}`}>{cta} <Icon name="arrow"/></Link></article>)}</div>
  </section>
