...
export default function Familias(){
  return <SiteShell>
    <section className="split-hero family-mobile-hero">
      <div>
        <span className="kicker">ATENDIMENTO HUMANIZADO</span>
        <h1>A família também precisa ser ouvida.</h1>
      ...
      <div className="family-page-photo" role="img" aria-label="Família em conversa acolhedora"/>
    </section>
    ...
    <section className="section-pad content-page family-content">
      ...
      <div className="service-grid family-service-grid">
        {services.map(([title,text],i)=><article key={title}><span>0{i+1}</span><h3>{title}</h3><p>{text}</p></article>)}
      </div>

      <section className="do-dont family-guidance">
        <div>
          <h2>O que pode ajudar</h2>
          <ul>
            <li><Icon name="check"/> Escolher um momento mais seguro para conversar</li>
            <li><Icon name="check"/> Falar sobre fatos e consequências, sem humilhar</li>
            <li><Icon name="check"/> Definir limites possíveis de manter</li>
            <li><Icon name="check"/> Buscar apoio para a própria família</li>
          </ul>
        </div>
        <div>
          <h2>O que costuma piorar</h2>
          <ul>
            <li>× Discutir durante intoxicação ou crise</li>
            <li>× Fazer ameaças que não serão cumpridas</li>
            <li>× Esconder todas as consequências do uso</li>
            <li>× Tentar controlar tudo sem ajuda</li>
          </ul>
        </div>
      </section>

      <div className="warm-callout family-callout">
        <span className="kicker">SEM CULPA, SEM PROMESSAS FÁCEIS</span>
        <h2>Você pode oferecer ajuda sem assumir sozinho a responsabilidade pela mudança.</h2>
        <p>Recuperação é um processo. Acolhimento e limites podem coexistir.</p>
      </div>
    </section>
  </SiteShell>
}
